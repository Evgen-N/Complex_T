...
class ComplexRNNLayer(nn.Module):
    """
    Комплексный RNN-слой на базе обычного GRU:
    - на входе раздельно r_seq, i_seq: [B, T, H]
    - внутри склеиваем их в [B, T, 2H] и гоняем через GRU
    - на выходе снова делим на (real, imag): [B, T, H], [B, T, H]
    """
    def __init__(self, hidden_dim: int, residual: bool = True, dropout: float = 0.0):
        super().__init__()
    ...
    def forward(self, r_seq: torch.Tensor, i_seq: torch.Tensor):
        # r_seq, i_seq: [B, T, H]
        B, T, H = r_seq.shape
        # склеиваем real + imag вдоль признаков
        x = torch.cat([r_seq, i_seq], dim=-1)  # [B, T, 2H]

        # пропуск через GRU
        out, _ = self.gru(x)                  # [B, T, 2H]
        out = self.dropout(out)

        # residual по последовательности (если нужно)
        if self.residual:
            out = out + x                     # [B, T, 2H]

        # делим обратно на real / imag
        r_out, i_out = out[..., :H], out[..., H:]  # [B, T, H], [B, T, H]
        return r_out, i_out
...
class ComplexCNNLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.rnn_layers = nn.ModuleList([
            ComplexRNNLayer(hidden_dim, residual=True, dropout=dropout)
